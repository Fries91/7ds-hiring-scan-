...
(function () {
  ...
  GM_addStyle(`
    /* ===== Briefcase Button (smaller, always visible) ===== */
    #${EL_BTN}{
      position: fixed;
      z-index: 2147483647;
  ...
  // ---------------- UI Nodes (icon cannot "disappear" now) ----------------
  ...
  setInterval(() => {
    ...
    if (!document.getElementById(EL_TOAST)) {
      try { document.documentElement.appendChild(toast); } catch {}
    }
    if (!document.getElementById(EL_PANEL)) {
      try { document.documentElement.appendChild(panel); } catch {}
    }
  }, 2000);
  ...
  // Badge button: tap toggles, drag moves
  ...
  function viewRecruit() {
    ...
    // initial fill from /state if present
  ...
  function render() {
    ...
    // Drag panel ONLY from header (tabs always clickable)
  ...
  function toggle(open) {
    ...
    btn.style.display = "flex"; // always show briefcase
