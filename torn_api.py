...
def hof_scan_workstats(api_key: str, max_pages: int = 10, page_size: int = 25) -> List[Dict[str, Any]]:
    ...
    for _ in range(int(max_pages)):
        ...
        for e in entries:
            ...
            try: man = int(man)
            except Exception: man = 0
            try: inte = int(inte)
            except Exception: inte = 0
            try: endu = int(endu)
            except Exception: endu = 0
            try: total = int(total)
            except Exception: total = man + inte + endu
            ...
            out.append(
                {"id": str(pid), "name": str(name), "man": man, "int": inte, "end": endu, "total": total}
            )
